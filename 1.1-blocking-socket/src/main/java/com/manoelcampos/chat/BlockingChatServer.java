...
//    Método que dá mome ao cliente
    public void namingClient(final ClientSocket clientSocket){
        if(clientSocket.login == null){
            clientSocket.login = clientSocket.getMessage();
        }
    }
    ...
    private void clientMessageLoop(final ClientSocket clientSocket){
        try {
            ...
            while((msg = clientSocket.getMessage()) != null){  
                ...
                System.out.println("Mensagem recebida do cliente "+ clientSocket.login +": " + msg);
                if(matchFound == true){
                    user = matcher.replaceAll(aux);
                    sendPrivateMsg(clientSocket, msg, user);
                }else{
    ...
    private void sendPrivateMsg(final ClientSocket sender, final String msg, final String user) {
        final Iterator<ClientSocket> iterator = clientSocketList.iterator();
        int count = 0;
        ...
        while (iterator.hasNext()) {
            ...
            /*Verifica se o elemento atual da lista (cliente) não é o cliente que enviou a mensagem.
            Se não for, encaminha a mensagem pra tal cliente.*/
            if(client.getMessage().equals(user)){
                if (!client.equals(sender)) {
                    if(client.sendMsg(msg))
                    count++;
                else iterator.remove();
